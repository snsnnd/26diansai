 ...
 *
 * PIT 定时器、UART、EXTI 等外设中断使用逐飞库的回调机制。
 ********************************************************************************************************************/
...
void GROUP1_IRQHandler (void)
{
    ...
    uint8 exti_group = 0;
    uint8 exti_event = 0;
    uint16 callback_index = 0;

    uint32  register_temp = 0;

    while((register_temp = gpio_group[0]->CPU_INT.IIDX) != 0)
    {
        exti_group = 0;
        ...
        callback_index = exti_group * GPIO_GROUP_PIN_NUMBER_MAX + exti_index;
        if(exti_callback_list[callback_index])
        {
            exti_callback_list[callback_index](exti_event, exti_callback_ptr_list[callback_index]);
        }
    }

    while((register_temp = gpio_group[1]->CPU_INT.IIDX) != 0)
    {
        exti_group = 1;
        exti_index = register_temp - 1;

...
static uint32 debug_format_unsigned(char *buff, uint32 value, uint32 base, uint8 uppercase)
{
    char temp[16];
...
static uint32 debug_vsnprintf(char *buff, uint32 buff_size, const char *format, va_list args)
{
    uint32 len = 0;

    while(*format && (len + 1 < buff_size))
    {
        if('%' != *format)
        {
            buff[len ++] = *format ++;
        ...
        switch(*format)
        {
            case 'c':
            {
                buff[len ++] = (char)va_arg(args, int);
            }break;

            case 'd':
            case 'i':
            {
                int32 value = long_format ? (int32)va_arg(args, long) : va_arg(args, int);
                if(0 > value)
                {
                    if(len + 1 < buff_size)
                    {
                ...
                number_len = debug_format_unsigned(number, (uint32)value, 10, 0);
            }break;

            case 'u':
            {
                uint32 value = long_format ? (uint32)va_arg(args, unsigned long) : va_arg(args, unsigned int);
                number_len = debug_format_unsigned(number, value, 10, 0);
            }break;
            ...
            {
                uint32 value = long_format ? (uint32)va_arg(args, unsigned long) : va_arg(args, unsigned int);
                number_len = debug_format_unsigned(number, value, 16, ('X' == *format));
            }break;

            case 'o':
            {
                uint32 value = long_format ? (uint32)va_arg(args, unsigned long) : va_arg(args, unsigned int);
                number_len = debug_format_unsigned(number, value, 8, 0);
            }break;

            case 'p':
            {
                uint32 value = (uint32)va_arg(args, void *);
                number_len = debug_format_unsigned(number, value, 16, 0);
            }break;
        ...
        for(uint32 i = 0; (i < number_len) && (len + 1 < buff_size); i ++)
        {
            buff[len ++] = number[i];
        }
...
int printf(const char *format, ...)
{
    ...
    len = debug_vsnprintf(output_buffer, sizeof(output_buffer), format, args);
    va_end(args);

    if(zf_debug_init_flag)
    {
        uart_write_buffer(DEBUG_UART_INDEX, (const uint8 *)output_buffer, len);
    }
...
void debug_init (void)
{
    ...
    uart_set_interrupt_config(DEBUG_UART_INDEX, UART_INTERRUPT_CONFIG_RX_ENABLE);
	  interrupt_set_priority(DEBUG_UART_PRIORITY, DEBUG_UART_PRIORITY0_7);        // 设置对应 DEBUG_UART_INDEX 的中断优先级

...
static void fifo_unlock (uint32 primask)
{
    if(0u == primask)
    {
        __enable_irq();
...
static void fifo_size_add (fifo_struct *fifo, uint32 amount)
{
    uint32 primask = fifo_lock();
    fifo->size += amount;
    fifo_unlock(primask);
}

static void fifo_size_subtract (fifo_struct *fifo, uint32 amount)
{
    uint32 primask = fifo_lock();
    fifo->size -= amount;
    fifo_unlock(primask);
...
uint32 fifo_used (fifo_struct *fifo)
{
    ...
    free_size = fifo->size;
    fifo_unlock(primask);
    return (fifo->max - free_size);                                             // 返回当前 FIFO 缓冲区中数据个数
}
...
fifo_state_enum fifo_write_element (fifo_struct *fifo, uint32 dat)
{
    ...
    do
    {
        ...
        if(1 <= fifo->size)                                                     // 剩余空间足够装下本次数据
        {
            ...
            fifo_size_subtract(fifo, 1);                                        // 缓冲区剩余长度减小
        }
        else
        {
...
fifo_state_enum fifo_write_buffer (fifo_struct *fifo, void *dat, uint32 length)
{
    ...
    do
    {
        ...
        if(length <= fifo->size)                                                // 剩余空间足够装下本次数据
        {
            ...
            fifo_size_subtract(fifo, length);                                   // 缓冲区剩余长度减小
        }
        else
        {
            return_state = FIFO_SPACE_NO_ENOUGH;                                // 当前 FIFO 缓冲区满 不能再写入数据 返回空间不足
...
fifo_state_enum fifo_read_buffer (fifo_struct *fifo, void *dat, uint32 *length, fifo_operation_enum flag)
{
    ...
    do
    {
        ...
        if(FIFO_READ_AND_CLEAN == flag)                                         // 如果选择读取并更改 FIFO 状态
        {
            ...
            fifo_size_add(fifo, *length);                                       // 释放对应长度空间
            fifo->execution &= ~FIFO_CLEAR;                                     // 清空作复位
        }
    }while(0);
...
fifo_state_enum fifo_read_tail_buffer (fifo_struct *fifo, void *dat, uint32 *length, fifo_operation_enum flag)
{
    ...
    do
    {
        ...
        else
        {
            ...
            fifo->execution |= FIFO_READ;                                       // 读操作置位
            head_snapshot = fifo->head;
            temp_length = (head_snapshot + fifo->max - *length) % fifo->max;
            fifo_data_length = fifo->max - temp_length;
            if(fifo_data_length > *length)
            {
                fifo_data_length = *length;
            }
            if(fifo_data_length == *length)
            {
                switch(fifo->type)
                {
                    case FIFO_DATA_8BIT:    memcpy(dat, &(((uint8 *)fifo->buffer)[temp_length]), *length);      break;
                    case FIFO_DATA_16BIT:   memcpy(dat, &(((uint16 *)fifo->buffer)[temp_length]), *length * 2); break;
                    case FIFO_DATA_32BIT:   memcpy(dat, &(((uint32 *)fifo->buffer)[temp_length]), *length * 4); break;
                }
            }
            else
            {
                switch(fifo->type)
                {
                    case FIFO_DATA_8BIT:
                    {
                        memcpy(dat, &(((uint8 *)fifo->buffer)[temp_length]), fifo_data_length);
                        memcpy(&(((uint8 *)dat)[fifo_data_length]), fifo->buffer, *length - fifo_data_length);
                    }break;
                    case FIFO_DATA_16BIT:
                    {
                        memcpy(dat, &(((uint16 *)fifo->buffer)[temp_length]), fifo_data_length * 2);
                        memcpy(&(((uint16 *)dat)[fifo_data_length]), fifo->buffer, (*length - fifo_data_length) * 2);
                    }break;
                    case FIFO_DATA_32BIT:
                    {
                        memcpy(dat, &(((uint32 *)fifo->buffer)[temp_length]), fifo_data_length * 4);
                        memcpy(&(((uint32 *)dat)[fifo_data_length]), fifo->buffer, (*length - fifo_data_length) * 4);
                    }break;
                }
            }
            fifo->execution &= ~FIFO_READ;                                      // 读操作复位

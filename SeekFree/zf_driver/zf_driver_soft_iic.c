...
static void soft_iic_set_error (soft_iic_info_struct *soft_iic_obj, soft_iic_status_enum error)
{
    if(SOFT_IIC_STATUS_OK == soft_iic_obj->last_error)
    {
        soft_iic_obj->last_error = error;
    }
}

static uint8 soft_iic_release_scl (soft_iic_info_struct *soft_iic_obj)
{
    vuint32 time_out = SOFT_IIC_CLOCK_STRETCH_TIMEOUT;

    gpio_high(soft_iic_obj->scl_pin);
    while(!gpio_get_level((gpio_pin_enum)soft_iic_obj->scl_pin))
    {
        if(0u == --time_out)
        {
            soft_iic_set_error(soft_iic_obj, SOFT_IIC_STATUS_CLOCK_STRETCH_TIMEOUT);
            return 0;
        }
    }
    return 1;
...
static uint8 soft_iic_recover_bus (soft_iic_info_struct *soft_iic_obj)
{
    uint8 clock_count;

    gpio_high(soft_iic_obj->sda_pin);
    if(!soft_iic_release_scl(soft_iic_obj))
    {
        return 0;
    }

    for(clock_count = 0u; clock_count < 9u; clock_count++)
    {
        gpio_low(soft_iic_obj->scl_pin);
    ...
    if(!gpio_get_level((gpio_pin_enum)soft_iic_obj->scl_pin)
        || !gpio_get_level((gpio_pin_enum)soft_iic_obj->sda_pin))
    {
        soft_iic_set_error(soft_iic_obj, SOFT_IIC_STATUS_BUS_STUCK);
        return 0;
    }
    return 1;
}
...
void soft_iic_start (soft_iic_info_struct *soft_iic_obj)
{
    ...
    if(!soft_iic_obj->transaction_active)
    {
        soft_iic_obj->last_error = SOFT_IIC_STATUS_OK;
        if(soft_iic_obj->addr > 0x7Fu)
        {
            soft_iic_obj->last_error = SOFT_IIC_STATUS_INVALID_PARAM;
            return;
        }
        if((!gpio_get_level((gpio_pin_enum)soft_iic_obj->scl_pin)
                || !gpio_get_level((gpio_pin_enum)soft_iic_obj->sda_pin))
            && !soft_iic_recover_bus(soft_iic_obj))
        {
            return;
        }
        soft_iic_obj->transaction_active = 1u;
    }
    else if(SOFT_IIC_STATUS_OK != soft_iic_obj->last_error)
    {
        return;
    }
    gpio_high(soft_iic_obj->sda_pin);                                           // SDA 高电平
    soft_iic_delay(soft_iic_obj->delay);
    if(!soft_iic_release_scl(soft_iic_obj))                                     // SCL 高电平
    {
        return;
    }
    soft_iic_delay(soft_iic_obj->delay);
...
void soft_iic_stop (soft_iic_info_struct *soft_iic_obj)
{
    ...
    soft_iic_delay(soft_iic_obj->delay);
    if(!soft_iic_release_scl(soft_iic_obj))                                     // SCL 先拉高
    {
        gpio_high(soft_iic_obj->sda_pin);
        soft_iic_obj->transaction_active = 0;
        return;
    }
    soft_iic_delay(soft_iic_obj->delay);
...
void soft_iic_send_ack (soft_iic_info_struct *soft_iic_obj, uint8 ack)
{
    ...
    soft_iic_delay(soft_iic_obj->delay);
    if(!soft_iic_release_scl(soft_iic_obj))                                     // SCL 拉高
    {
        gpio_high(soft_iic_obj->sda_pin);
        return;
    }
...
//        gpio_set_level(soft_iic_obj->sda_pin, data & temp);
    ...
    ack = ((soft_iic_wait_ack(soft_iic_obj) == 1) ? 0 : 1);
    if(!ack && (SOFT_IIC_STATUS_OK == soft_iic_obj->last_error))
    {
        soft_iic_obj->last_error = SOFT_IIC_STATUS_NACK;
    }
    return ack;
}
...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
soft_iic_status_enum soft_iic_write_8bit_register_checked (soft_iic_info_struct *soft_iic_obj, const uint8 register_name, const uint8 data)
{
    return soft_iic_write_8bit_registers_checked(soft_iic_obj, register_name, &data, 1);
}

soft_iic_status_enum soft_iic_write_8bit_registers_checked (soft_iic_info_struct *soft_iic_obj, const uint8 register_name, const uint8 *data, uint32 len)
{
    if(NULL == soft_iic_obj)
    {
        return SOFT_IIC_STATUS_INVALID_PARAM;
    }
    if((soft_iic_obj->addr > 0x7F) || (NULL == data) || (0 == len))
    {
        soft_iic_obj->last_error = SOFT_IIC_STATUS_INVALID_PARAM;
        return soft_iic_obj->last_error;
    }
    ...
    while(len --)
    {
        if(!soft_iic_send_data(soft_iic_obj, *data ++))
        {
            soft_iic_stop(soft_iic_obj);
            return soft_iic_obj->last_error;
        }
...
{
    zf_assert(NULL != soft_iic_obj);
    (void)soft_iic_write_8bit_register_checked(soft_iic_obj, register_name, data);
}
...
void soft_iic_write_8bit_registers (soft_iic_info_struct *soft_iic_obj, const uint8 register_name, const uint8 *data, uint32 len)
{
    zf_assert(NULL != soft_iic_obj);
    zf_assert(NULL != data);
    (void)soft_iic_write_8bit_registers_checked(soft_iic_obj, register_name, data, len);
}
...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
soft_iic_status_enum soft_iic_read_8bit_register_checked (soft_iic_info_struct *soft_iic_obj, const uint8 register_name, uint8 *data)
{
    return soft_iic_read_8bit_registers_checked(soft_iic_obj, register_name, data, 1);
}

soft_iic_status_enum soft_iic_read_8bit_registers_checked (soft_iic_info_struct *soft_iic_obj, const uint8 register_name, uint8 *data, uint32 len)
{
    if(NULL == soft_iic_obj)
    {
        return SOFT_IIC_STATUS_INVALID_PARAM;
...
uint8 soft_iic_read_8bit_register (soft_iic_info_struct *soft_iic_obj, const uint8 register_name)
{
    uint8 temp = 0;
    zf_assert(NULL != soft_iic_obj);
    (void)soft_iic_read_8bit_register_checked(soft_iic_obj, register_name, &temp);
    return temp;
}
...
void soft_iic_read_8bit_registers (soft_iic_info_struct *soft_iic_obj, const uint8 register_name, uint8 *data, uint32 len)
{
    zf_assert(NULL != soft_iic_obj);
    zf_assert(NULL != data);
    (void)soft_iic_read_8bit_registers_checked(soft_iic_obj, register_name, data, len);
}
...
void soft_iic_write_splicing_array (soft_iic_info_struct *soft_iic_obj, const uint8 *part1_data, uint32 part1_len, const uint8 *part2_data, uint32 part2_len)
{
    ...
    (void)soft_iic_write_splicing_array_checked(soft_iic_obj,
        part1_data, part1_len, part2_data, part2_len);
}

soft_iic_status_enum soft_iic_write_splicing_array_checked (soft_iic_info_struct *soft_iic_obj, const uint8 *part1_data, uint32 part1_len, const uint8 *part2_data, uint32 part2_len)
{
    if(NULL == soft_iic_obj)
    {
        return SOFT_IIC_STATUS_INVALID_PARAM;
    }
    if((soft_iic_obj->addr > 0x7Fu) || ((0u == part1_len) && (0u == part2_len))
        || ((0u != part1_len) && (NULL == part1_data))
        || ((0u != part2_len) && (NULL == part2_data)))
    {
        soft_iic_obj->last_error = SOFT_IIC_STATUS_INVALID_PARAM;
        return soft_iic_obj->last_error;
    }
    ...
    while(part1_len --)
    {
        if(!soft_iic_send_data(soft_iic_obj, *part1_data ++))
        {
            soft_iic_stop(soft_iic_obj);
            return soft_iic_obj->last_error;
        }
    ...
    while(part2_len --)
    {
        if(!soft_iic_send_data(soft_iic_obj, *part2_data ++))
        {
            soft_iic_stop(soft_iic_obj);
            return soft_iic_obj->last_error;
        }
...
uint8 soft_iic_sccb_read_register (soft_iic_info_struct *soft_iic_obj, const uint8 register_name)
{
    zf_assert(NULL != soft_iic_obj);
    uint8 temp = 0;
    soft_iic_status_enum write_status;
    soft_iic_start(soft_iic_obj);
    soft_iic_send_data(soft_iic_obj, soft_iic_obj->addr << 1);
    soft_iic_send_data(soft_iic_obj, register_name);
    soft_iic_stop(soft_iic_obj);
    write_status = soft_iic_obj->last_error;

    soft_iic_start(soft_iic_obj);
    soft_iic_send_data(soft_iic_obj, soft_iic_obj->addr << 1 | 0x01);
    temp = soft_iic_read_data(soft_iic_obj, 1);
    soft_iic_stop(soft_iic_obj);
    if(SOFT_IIC_STATUS_OK != write_status)
    {
        soft_iic_obj->last_error = write_status;
    }
    return temp;
}
...
void soft_iic_init (soft_iic_info_struct *soft_iic_obj, uint8 addr, uint32 delay, gpio_pin_enum scl_pin, gpio_pin_enum sda_pin)
{
    ...
    soft_iic_obj->last_error = (addr > 0x7F) ? SOFT_IIC_STATUS_INVALID_PARAM : SOFT_IIC_STATUS_OK;
    soft_iic_obj->transaction_active = 0;
    gpio_init(scl_pin, GPO, GPIO_HIGH, GPO_OPEN_DTAIN);                         // SCL 释放高电平并允许时钟延展
    gpio_init(sda_pin, GPO, GPIO_HIGH, GPO_OPEN_DTAIN);                         // 提取对应IO索引 AF功能编码
    if(!gpio_get_level(scl_pin) || !gpio_get_level(sda_pin))
    {
        (void)soft_iic_recover_bus(soft_iic_obj);
    }
}

soft_iic_status_enum soft_iic_get_last_error (const soft_iic_info_struct *soft_iic_obj)
{
    return (NULL == soft_iic_obj) ? SOFT_IIC_STATUS_INVALID_PARAM : soft_iic_obj->last_error;
}

...
static  void exti_callbakc_defalut (uint32 event, void *ptr);
        void_callback_uint32_ptr exti_callback_list[GPIO_GROUP_MAX * GPIO_GROUP_PIN_NUMBER_MAX];
        void *exti_callback_ptr_list[GPIO_GROUP_MAX * GPIO_GROUP_PIN_NUMBER_MAX];
...
void exti_enable (gpio_pin_enum pin)
{
    ...
    gpio_group[io_group]->CPU_INT.IMASK |= (1u << io_pin);
...
void exti_disable (gpio_pin_enum pin)
{
    ...
    if(!gpio_group[0]->CPU_INT.IMASK && !gpio_group[1]->CPU_INT.IMASK)
    {
...
// 备注信息     GPIOA/GPIOB 各自有独立 CPU_INT 状态和回调槽；同号 A/B 引脚可共用 GROUP1 向量
//-------------------------------------------------------------------------------------------------------------------
void exti_init (gpio_pin_enum pin, exti_trigger_enum trigger, void_callback_uint32_ptr callback, void *ptr)
{
    ...
    exti_callback_ptr_list[io_group * GPIO_GROUP_PIN_NUMBER_MAX + io_pin]  = ptr;
    exti_callback_list[io_group * GPIO_GROUP_PIN_NUMBER_MAX + io_pin]      = (NULL == callback) ? (exti_callbakc_defalut) : (callback);
    ...
    gpio_group[io_group]->CPU_INT.ICLR  |= (1u << io_pin);
    gpio_group[io_group]->CPU_INT.IMASK |= (1u << io_pin);

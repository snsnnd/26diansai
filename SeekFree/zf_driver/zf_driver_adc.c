...
uint16 adc_convert (adc_pin_enum adc_pin)
{
    ...
    (void)adc_convert_checked(adc_pin, &return_value);
    return return_value;
}

uint8 adc_convert_checked (adc_pin_enum adc_pin, uint16 *result)
{
    uint8   adc_index       = (adc_pin >> ADC_INDEX_OFFSET) & ADC_INDEX_MASK;
    uint8   adc_channel     = (adc_pin >> ADC_CHANNEL_OFFSET) & ADC_CHANNEL_MASK;
    vuint32 time_out        = ADC_CONVERSION_TIMEOUT;

    if((ADC_NUM <= adc_index) || (NULL == result))
    {
        return ZF_ERROR;
    }
    *result = 0;
    ...
    DL_ADC12_clearInterruptStatus(adc_list[adc_index], DL_ADC12_INTERRUPT_MEM0_RESULT_LOADED);
    DL_ADC12_startConversion(adc_list[adc_index]);
    while((0u == DL_ADC12_getRawInterruptStatus(adc_list[adc_index], DL_ADC12_INTERRUPT_MEM0_RESULT_LOADED))
        && (0u != time_out))
    {
        time_out--;
    }
    ...
    *result = DL_ADC12_getMemResult(adc_list[adc_index], DL_ADC12_MEM_IDX_0);
    DL_ADC12_clearInterruptStatus(adc_list[adc_index], DL_ADC12_INTERRUPT_MEM0_RESULT_LOADED);
    return ZF_NO_ERROR;
}

...
static uint8 flash_wait_for_command (void)
{
    vuint32 time_out = FLASH_OPERATION_TIME_OUT;
    DL_FLASHCTL_COMMAND_STATUS status;

    do
    {
        status = (DL_FLASHCTL_COMMAND_STATUS)FLASHCTL->GEN.STATCMD;
        if(DL_FLASHCTL_COMMAND_STATUS_IN_PROGRESS != status)
        {
            break;
        }
        time_out--;
    ...
    return (DL_FLASHCTL_COMMAND_STATUS_PASSED == status) ? ZF_NO_ERROR : ZF_ERROR;
}
...
uint8 flash_check (uint32 sector_num, uint32 page_num)
{
    ...
    for(temp_loop = 0; FLASH_PAGE_SIZE > temp_loop; temp_loop += sizeof(uint32))// 循环读取 Flash 的值
    {
...
uint8 flash_erase_page (uint32 sector_num, uint32 page_num)
{
    ...
    return_state = flash_wait_for_command();
    DL_FlashCTL_executeClearStatus(FLASHCTL);
...
void flash_read_page (uint32 sector_num, uint32 page_num, uint32 *buf, uint16 len)
{
    ...
    {
        *buf ++ = *(__IO uint32*)(flash_addr + temp_loop * sizeof(uint32));      // 循环读取 Flash 的值
    }
}
...
uint8 flash_write_page (uint32 sector_num, uint32 page_num, const uint32 *buf, uint16 len)
{
    ...
    {
        if(ZF_NO_ERROR != flash_erase_page(sector_num, page_num))                // 擦除这一页
        {
            return ZF_ERROR;
        }
    }
    ...
    while((uint16)(data_index + 1u) < len)                                      // 每个 Flash word 为 64bit
    {
        DL_FlashCTL_executeClearStatus(FLASHCTL);
        DL_FlashCTL_unprotectSector(FLASHCTL, flash_addr, DL_FLASHCTL_REGION_SELECT_MAIN);
        FLASHCTL->GEN.CMDTYPE = (uint32_t)DL_FLASHCTL_COMMAND_SIZE_ONE_WORD |
            DL_FLASHCTL_COMMAND_TYPE_PROGRAM;
        FLASHCTL->GEN.CMDBYTEN = DL_FLASHCTL_PROGRAM_64_WITH_ECC;
        FLASHCTL->GEN.CMDADDR = flash_addr;
        FLASHCTL->GEN.CMDDATA0 = buf[data_index];
        FLASHCTL->GEN.CMDDATA1 = buf[data_index + 1u];
        FLASHCTL->GEN.CMDEXEC = FLASHCTL_CMDEXEC_VAL_EXECUTE;
        return_state = flash_wait_for_command();
        if(ZF_NO_ERROR != return_state)
        {
            break;
        }
        flash_addr += 8u;
    ...
    if((ZF_NO_ERROR == return_state) && (data_index < len))
    {
        DL_FlashCTL_executeClearStatus(FLASHCTL);
        DL_FlashCTL_unprotectSector(FLASHCTL, flash_addr, DL_FLASHCTL_REGION_SELECT_MAIN);
        FLASHCTL->GEN.CMDTYPE = (uint32_t)DL_FLASHCTL_COMMAND_SIZE_ONE_WORD |
...
void flash_read_page_to_buffer (uint32 sector_num, uint32 page_num)
{
    ...
    {
        flash_union_buffer[temp_loop].uint32_type = *(__IO uint32*)(flash_addr + temp_loop * sizeof(uint32));
    }
}
...
uint8 flash_write_page_from_buffer (uint32 sector_num, uint32 page_num)
{
    zf_assert(FLASH_MAX_SECTION_INDEX > sector_num);                            // 参数范围 0-63
    zf_assert(FLASH_MAX_PAGE_INDEX > page_num);                                 // 参数范围 0-3
    return flash_write_page(sector_num, page_num,
        (const uint32 *)flash_union_buffer, FLASH_DATA_BUFFER_SIZE);
}

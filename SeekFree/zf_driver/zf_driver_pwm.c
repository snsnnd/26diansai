...
void pwm_force_low (pwm_channel_enum pin)
{
    GPTIMER_Regs *timer_obj = timer_reg[(pin >> PWM_INDEX_OFFSET) & PWM_INDEX_MASK];
    uint32 channel_index = (pin >> PWM_CHANNEL_OFFSET) & PWM_CHANNEL_MASK;

    timer_obj->COMMONREGS.ODIS |=
        (GPTIMER_ODIS_C0CCP0_CCP_OUTPUT_LOW << channel_index);
}
...
void pwm_init (pwm_channel_enum pin, const uint32 freq, const uint32 duty)
{
    ...
    {
        timer_obj->COUNTERREGS.CCCTL_23[channel_index - 2] =
            ((PWM_CH2 == channel_index)
                ? GPTIMER_CCCTL_23_CC2SELU_SEL_CCU2 | GPTIMER_CCCTL_23_CC2SELD_SEL_CCD2
                : GPTIMER_CCCTL_23_CC2SELU_SEL_CCU3 | GPTIMER_CCCTL_23_CC2SELD_SEL_CCD3);
